...
function ExifImage (options) {
  if (!(this instanceof ExifImage)) {
    ...
    var exifImage = new ExifImage(options);
    return exifImage.exifData;
  }
...
ExifImage.prototype.loadImage = function (image) {
  ...
  if (image.constructor.name === 'Buffer') {
    return this.processImage("Buffer", image);
  }

  if (image.constructor.name === 'String') {
    try {
      var data = fs.readFileSync(image);
      return self.processImage("File: "+image, data);
    }
    catch(error) {
      throw new Error('Encountered the following error while trying to read given image: '+error);
    }
  }
...
ExifImage.prototype.processImage = function (source, data) {
