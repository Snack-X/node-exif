...
describe('node-exif API', function() {
  ...
  it('test constructor (filename)', function() {

    var ExifImage = require('..').ExifImage;

    var data = new ExifImage({image: path }).exifData;

    assert.equal(JSON.stringify(data), json, "Not same datas ?");
  });

  it('test constructor (buffer)', function() {

    var ExifImage = require('..').ExifImage;

    var buffer=fs.readFileSync(path);

    var data = new ExifImage({image: buffer }).exifData;

    assert.equal(JSON.stringify(data), json, "Not same datas ?");
  });

  it('test loadImage (filename)', function() {

    var ExifImage = require('..').ExifImage;

    var exif=new ExifImage();

    var data = exif.loadImage(path);

    assert.equal(JSON.stringify(data), json, "Not same datas ?");
  });

  it('test loadImage (buffer)', function() {
    ...
    var data = exif.loadImage(buffer);

    assert.equal(JSON.stringify(data), json, "Not same datas");
  });

  it('test wrapper', function() {
